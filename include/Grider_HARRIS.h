 ...
 */
#ifndef OV_CORE_GRIDER_HARRIS_H
#define OV_CORE_GRIDER_HARRIS_H
...
namespace USLAM {
    ...
    public:
        ...
        static void perform_griding(const cv::Mat &img, std::vector<cv::KeyPoint> &pts, int num_features,
            ...
            auto num_features_grid = (int) (num_features / (grid_x * grid_y)) + 1; //how many number of points should be extracted froom each box
            ...
            parallel_for_(cv::Range(0, ct_cols*ct_rows), [&](const cv::Range& range) {
                for (int r = range.start; r < range.end; r++) {
                    ...
                    std::vector<cv::KeyPoint> ptsH_new;
                    //cv::FAST(img(img_roi), pts_new, threshold, nonmaxSuppression);

                    vector<cv::Point2f> corners;
                    double qualityLevel = 0.001; //corner point detection acceptable minimum eigenvalue
	                double minDistance = 8;//minimum distance between corner points
                    int blockSize = 3; //calculate the neighborhood range specified by the derivative autocorrelation matrix
                    double k = 0.04; //weight coefficient
                    cv::goodFeaturesToTrack(img(img_roi),corners,num_features_grid,qualityLevel,minDistance ,cv::Mat(),blockSize,true,k);
	                
                    //getting SUbpixel Accuracy
                    ...
                    //cv::cornerSubPix(img(img_roi), corners, winSize, zeroZone, criteria);
                    cv::KeyPoint::convert(corners, ptsH_new);
                    ...
                    //for (size_t i = 0; i < (size_t) num_features_grid && i < ptsH_new.size(); i++) {
                    for (size_t i = 0; i < ptsH_new.size(); i++) {
                        cv::KeyPoint pt_cor = ptsH_new.at(i);
                        pt_cor.pt.x += x;
                        pt_cor.pt.y += y;
